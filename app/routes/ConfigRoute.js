...
var routesApiConfig = [
    ...
    },
    {
        ...
        middleware: [function(req, res) {
            console.log("GET - /config/public");
            var keys = ['rights','support'],config = {};
                ...
                case 'start' :
                    fsExtra.copy(path.resolve('./node_modules/newrelic/newrelic.js'), 'newrelic.js', { replace: false }, function (err) {
                        ...
                        replace({
                            ...
                            replacement: '',
                            paths: [global.CONFIG.server.node.newFile]
                        });

                        prependFile(global.CONFIG.server.node.newFile,"require('newrelic');", function(err) {
                            if (err) {
                                return Response.build(res,500,{data : err});
                            } else {
                                setTimeout(function(){
                                ...
                                return Response.build(res,200,{data : 'success'});
                            }
                        });
    ...
    },
    {
        ...
        middleware: [function(req, res) {
            ...
            fs.readFile(global.CONFIG.server.node.newFile, "utf-8", function(err, data) {
                if(!err && data.indexOf("require('newrelic')") >= 0 ){
